...
CONFIG_FILE="$HOME/.kAwOkenrcWhite"
ICONSET="kAwOkenWhite"
...
changefolder()
{
  ...
  ICNST=$3
  ICONSDIR=$4
  HOMEDIR="`kde4-config --localprefix`share/icons/$ICNST"
  ...
  cd $ICONSDIR/clear/22x22/places/$TYPE/

  for f in *; do
  	cd $HOMEDIR/clear/22x22/places
  	ln -fs $TYPE/$f $f
  	cd $ICONSDIR/clear/22x22/places/$TYPE/
  done
  ...
  if [ "$TYPE" = "s11/s11" ] || [ "$TYPE" = "s11/s11-original" ]; then
    cd $HOMEDIR/clear/128x128/places/$TYPE
  	...
  	cd $HOMEDIR/clear/22x22/places/$TYPE
  	ln -fs ../s11-folders/$SUBTYPE".png" "folder.png"
  fi
