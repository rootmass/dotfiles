...
echo "*************************************************************************"
if [ -x /usr/bin/zenity ]; then 
  QST="Due to the highly experimental nature of the script,\nI recommend you to check the README pdf file\nto fully understand what you have to know\nto use it. Do you want to read the PDF file?"
  ...
  echo "Due to the highly experimental script, I recommend you to check the README pdf file to fully understand what you have to know to use it. Do you want to read the PDF file? ([y]/n)"
  read RISP
  if [ "$RISP" != "n" ];then
    RISP="Yes"
  fi
...
echo "*************************************************************************"
if ! [ -x /usr/bin/zenity ]; then
  echo "How do you want to colorize the iconset? You have to choose a value for each CMY channel (check the PDF file for further informations). For each channel, type the number of choice and press 'Enter'."
  echo "Quick suggestions: the number must be in the range [0 99]; higher is the number, more contrasted is the result (and, generally, less details in icons you'll get). Type now the numbers as specified earlier:"
fi

f="0"
while [  $f == "0" ]; do
  if [ -x /usr/bin/zenity ]; then 
    R=`zenity --title="$ICNSTG $VER Customization script" --entry --text="How do you want to colorize the iconset?\nYou have to choose a value for each CMY channel (check the PDF file for info).\nFor each channel, type the number of choice and press 'OK'.\n\nQuick suggestions:\n1. the number must be in the range [0 99];\n2. higher is the number, more contrasted is the result (and less details you'll get).\n\nType now the C channel as specified earlier:" --width=100`
  else
    read R
  fi
  if [ ! $(echo "$R" | grep -E "^[0-9]+$") ]; then
    echo "$R is not a valid integer. Type a correct number for C channel."
  else
    if ! [ $R -ge 0 ] || ! [ $R -le 99 ]; then
      echo "$R is an invalid value, because range is [0 99]. Type a correct number for C channel."
    else
      f="1"
    fi
  fi
...
while [  $f = "0" ]; do
  if [ -x /usr/bin/zenity ]; then 
    G=`zenity --title="$ICNSTG $VER Customization script" --entry --text="How do you want to colorize the iconset?\nYou have to choose a value for each CMY channel (check the PDF file for info).\nFor each channel, type the number of choice and press 'OK'.\n\nQuick suggestions:\n1. the number must be in the range [0 99];\n2. higher is the number, more contrasted is the result (and less details you'll get).\n\nType now the M channel as specified earlier:" --width=100`
  else
    read G
  fi
  if [ ! $(echo "$G" | grep -E "^[0-9]+$") ]; then
    echo "$G is not a valid integer. Type a correct number for M channel."
  else
    if ! [ $G -ge 0 ] || ! [ $G -le 99 ]; then
      echo "$G is an invalid value, because range is [0 99]. Type a correct number for M channel."
    else
      f="1"
    fi
  fi
...
while [  $f = "0" ]; do
  if [ -x /usr/bin/zenity ]; then 
    B=`zenity --title="$ICNSTG $VER Customization script" --entry --text="How do you want to colorize the iconset?\nYou have to choose a value for each CMY channel (check the PDF file for info).\nFor each channel, type the number of choice and press 'OK'.\n\nQuick suggestions:\n1. the number must be in the range [0 99];\n2. higher is the number, more contrasted is the result (and less details you'll get).\n\nType now the Y channel as specified earlier:" --width=100`
  else
    read B
  fi
  if [ ! $(echo "$B" | grep -E "^[0-9]+$") ]; then
    echo "$B is not a valid integer. Type a correct number for Y channel."
  else
    if ! [ $B -ge 0 ] || ! [ $B -le 99 ]; then
      echo "$B is an invalid value, because range is [0 99]. Type a correct number for Y channel."
    else
      f="1"
    fi
  fi
...
sed -i 's/Awesome Token/Awesome Token (colored with '${color:0:2}'\/'${color:3:2}'\/'${color:6:2}')/g' index.theme
